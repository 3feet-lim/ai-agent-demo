...
def build_resolve_prompt(extracted: dict, profile: str, region: str) -> str:
    ...
    return "\n".join([
        ...
        "   → 클러스터명이 없는 경우:",
        "     a) 먼저 check_resources에 'aws eks list-clusters' 요청하여 클러스터 목록 확인.",
        "     b) 클러스터가 1개면 해당 클러스터 사용.",
        "     c) 클러스터가 여러 개면 collect_metrics에",
        '        \'container_restart_count{PodName=~"{이름}.*"} 또는',
        '         pod_number_of_container_restarts{PodName=~"{이름}.*"} 쿼리로 ClusterName 라벨 확인\' 요청.',
        "        (⚠️ kube_pod_info는 kube-state-metrics 전용이므로 사용 금지.",
        "         CloudWatch Container Insights 메트릭을 사용할 것.)",
