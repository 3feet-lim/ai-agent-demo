...
def build_log_agent_prompt() -> str:
    ...
    return "\n".join([
        ...
        "- 이전 단계에서 확인된 로그 그룹이 여러 개이면 모든 로그 그룹을 하나도 빠짐없이 각각 조회할 것.",
        "  일부만 골라서 조회하지 말 것. 전부 조회해야 함.",
        "- 로그 그룹명을 추측하지 말 것.",
        "  task에 이전 단계에서 확인된 로그 그룹 이름이 있으면 그 이름만 사용.",
        "  이름이 없으면 describe_log_groups로 탐색.",
        "",
        "## 시간 범위 규칙",
        "- task에 이벤트 발생 시각이 명시되어 있으면, 해당 시각 전후를 중심으로 로그를 조회할 것.",
        "  예: 발생 시각이 14:20이면 14:10~14:30 범위로 조회.",
        "- task에 시간 범위가 명시되어 있으면 그 범위를 사용.",
        "- 둘 다 없으면 최근 30분.",
        "",
        "## 제한",
        "- 최대 30회 도구 호출.",
...
def build_resource_agent_prompt() -> str:
    ...
    return "\n".join([
        ...
        "- 🚨 EKS 클러스터의 로그 그룹은 두 가지 prefix에 존재하므로 반드시 두 번 조회할 것:",
        "  1) `/aws/eks/{클러스터명}` — 컨트롤 플레인 로그 (api-server, audit, authenticator 등)",
        "  2) `/aws/containerinsights/{클러스터명}` — Container Insights 로그 (application, dataplane, host, performance)",
        "  한쪽만 조회하면 로그 누락이 발생함. 두 prefix 모두 조회 필수.",
