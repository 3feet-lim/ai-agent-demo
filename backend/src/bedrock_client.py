...
class BedrockAgent:
    ...
    def _build_system_prompt(self, mcp_context: MCPContext) -> str:
        ...
        lines = [
            ...
            "|-----------|-----------|------------|",
            "| OOMKilled / memory | Grafana (memory) → AWS CLI (task/node) | CloudWatch (logs) |",
            "| High CPU | Grafana (CPU) | CloudWatch → AWS CLI (capacity) |",
            "| CrashLoopBackOff | CloudWatch (error logs) → Grafana (restarts) | AWS CLI (events) |",
            "| 5xx / latency | Grafana (error rate, latency) → CloudWatch | AWS CLI (target health) |",
            "| Node NotReady | AWS CLI (node group, ASG) → Grafana | CloudWatch (system logs) |",
            "| Deployment failure | AWS CLI (deploy status) → CloudWatch | Grafana (before/after) |",
            "| Disk pressure | Grafana (disk usage) → AWS CLI (EBS) | CloudWatch |",
            "| General status | Grafana (dashboards) → AWS CLI (resource list) | CloudWatch |",
            "",
