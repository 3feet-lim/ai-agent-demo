...
class BedrockAgent:
    ...
    def _build_system_prompt(self, mcp_context: MCPContext, enforced_time_window: tuple[str, str] | None = None) -> str:
        ...
        lines = [
            ...
            "- Grafana MCP: provides metric query tools (PromQL queries, dashboards, panels, etc.)",
            "  → ALWAYS prefer direct PromQL queries over dashboard/panel browsing.",
            "  → Use query_prometheus or equivalent PromQL tool to fetch metrics directly.",
            "  → Do NOT browse dashboards first. Construct the PromQL query based on the",
            "    metric name and labels you need, then query Prometheus directly.",
            "  → Only fall back to dashboard search if you cannot determine the correct",
            "    metric name or labels for the PromQL query.",
            ...
            "    → Use PromQL queries directly (e.g. query_prometheus tool).",
            "    → Do NOT start by listing/searching dashboards. Go straight to PromQL.",
            ...
            "",
            "### Resource Discovery (CRITICAL - NEVER give up without exploring)",
            "",
            "When investigating an issue, you MUST actively discover related resources.",
            "Do NOT assume you know all relevant log groups, metrics, or resources.",
            "ALWAYS explore before concluding 'cause unknown'.",
            "",
            "#### Step 0: Identify the Service Type",
            "Before any investigation, determine what AWS service is involved:",
            "- Check the alarm name, namespace, resource tags, or user message for clues.",
            "- Common patterns: EKS (kubernetes, pod, node, container), Lambda (function,",
            "  invocation, timeout), EC2 (instance, AMI), RDS (database, aurora, replica),",
            "  ALB/NLB (target group, listener, 5xx), S3, DynamoDB, etc.",
            "- Once identified, follow the service-specific discovery below.",
            "",
            "#### Step 1: Log Group Discovery (ALWAYS do this first)",
            "NEVER guess log group names. ALWAYS list/search first:",
            "- Use CloudWatch MCP to list log groups with a prefix filter.",
            "- Try multiple prefix patterns if the first returns nothing:",
            "  → /aws/{service-name}/, /aws/{service-name}/{resource-name}/",
            "  → /{environment}/, /{team-name}/, /{application-name}/",
            "- If no results, broaden the search or list all log groups and filter.",
            "",
            "#### Step 2: Service-Specific Discovery",
            "",
            "##### EKS (Kubernetes)",
            "Check ALL Container Insights log groups under /aws/containerinsights/{cluster}/:",
            "  1. application — Pod stdout/stderr, app errors, OOMKilled",
            "  2. dataplane — kubelet events, node conditions, scheduling, evictions",
            "  3. host — Node system logs, kernel messages, disk/memory pressure",
            "  4. performance — Container/pod/node performance metrics",
            "  5. flowlogs — VPC flow logs for network issues",
            "Also check control plane logs if enabled: /aws/eks/{cluster}/cluster",
            "  → API server, scheduler, controller-manager, authenticator",
            "For pod issues: application → dataplane → host (in order).",
            "For node issues: host → dataplane → performance (in order).",
            "For network issues: flowlogs → dataplane → application (in order).",
            "",
            "##### Lambda",
            "Log group pattern: /aws/lambda/{function-name}",
            "Key investigation points:",
            "  1. Invocation errors — search for 'ERROR', 'Task timed out', 'Runtime.ExitError'",
            "  2. Cold starts — search for 'INIT_START', 'Init Duration'",
            "  3. OOM — search for 'Runtime.ExitError', 'signal: killed', memory size",
            "  4. Timeout — search for 'Task timed out after', check duration vs timeout setting",
            "  5. Concurrency — check throttles via metrics, 'Rate exceeded', 'TooManyRequestsException'",
            "Also check: X-Ray traces (if enabled), Dead Letter Queue, destination config.",
            "",
            "##### EC2",
            "Log groups vary by setup. Search for:",
            "  - /var/log/messages, /var/log/syslog (system logs via CloudWatch agent)",
            "  - Custom application log groups configured by CloudWatch agent",
            "  - /aws/ec2/{instance-id} or similar patterns",
            "Also check: instance status checks, system status checks via AWS API MCP,",
            "  CloudTrail for recent API calls (stop/start/terminate/modify).",
            "",
            "##### RDS / Aurora",
            "Log group pattern: /aws/rds/instance/{db-id}/{log-type} or /aws/rds/cluster/{cluster-id}/{log-type}",
            "Log types: error, general, slowquery, audit, postgresql (varies by engine).",
            "Key investigation points:",
            "  1. Error log — connection errors, crash recovery, replication lag",
            "  2. Slow query log — long-running queries causing CPU/memory spikes",
            "  3. Check Enhanced Monitoring and Performance Insights via metrics",
            "  4. Failover events, storage full, parameter changes via AWS API MCP",
            "",
            "##### ALB / NLB",
            "Access logs are in S3 (not CloudWatch). For investigation:",
            "  1. Metrics first — 5xx count, target response time, healthy host count via Grafana",
            "  2. Target health — check target group health via AWS API MCP",
            "  3. Related service logs — check the backend service (EKS pods, EC2, Lambda)",
            "  4. CloudTrail — recent changes to listeners, rules, target groups",
            "",
            "##### Network Connectivity Troubleshooting",
            "When a user reports connectivity issues (timeout, unreachable, packet loss, etc.),",
            "follow this systematic investigation order using AWS API MCP (call_aws):",
            "",
            "Step A: Identify the network path",
            "  - Source and destination: which VPCs, subnets, instances, or services?",
            "  - Are they in the same VPC, cross-VPC, cross-region, or cross-account?",
            "  - What connectivity method? (VPC Peering, Transit Gateway, VPN, Direct Connect, Internet)",
            "",
            "Step B: Check routing (most common cause of connectivity failure)",
            "  1. VPC Route Tables — describe-route-tables for both source and destination subnets.",
            "     Verify the destination CIDR has a route pointing to the correct target",
            "     (igw, nat-gw, tgw, pcx, vgw, eni, etc.).",
            "  2. Transit Gateway (if applicable):",
            "     - describe-transit-gateways — TGW state",
            "     - describe-transit-gateway-attachments — attachment state (available?)",
            "     - describe-transit-gateway-route-tables — which route table is associated",
            "     - search-transit-gateway-routes — does a route exist for the destination CIDR?",
            "     - get-transit-gateway-route-table-associations — correct VPC attached?",
            "     - get-transit-gateway-route-table-propagations — routes propagated?",
            "  3. VPC Peering (if applicable):",
            "     - describe-vpc-peering-connections — peering state (active?)",
            "     - Both VPCs must have routes pointing to the peering connection",
            "  4. VPN / Direct Connect (if applicable):",
            "     - describe-vpn-connections — VPN tunnel status (UP?)",
            "     - describe-virtual-interfaces — DX virtual interface state",
            "",
            "Step C: Check security controls",
            "  1. Security Groups — describe-security-groups for source and destination ENIs.",
            "     Verify inbound rules on destination allow traffic from source (port, protocol, CIDR).",
            "     Verify outbound rules on source allow traffic to destination.",
            "  2. Network ACLs — describe-network-acls for both subnets.",
            "     NACLs are stateless: check BOTH inbound AND outbound rules, including rule numbers",
            "     (lower number = higher priority). Watch for explicit DENY rules.",
            "",
            "Step D: Check resource state",
            "  1. ENI status — describe-network-interfaces for relevant ENIs",
            "  2. Instance/service status — is the target actually running?",
            "  3. NAT Gateway (if applicable) — describe-nat-gateways, state and connectivity type",
            "  4. Internet Gateway — describe-internet-gateways, attached to VPC?",
            "",
            "Step E: Check flow logs and CloudTrail",
            "  1. VPC Flow Logs — search CloudWatch log groups for /vpc/flowlogs or similar.",
            "     Look for REJECT entries matching the source/destination IP and port.",
            "  2. CloudTrail — recent changes to route tables, security groups, NACLs, TGW,",
            "     peering connections, or VPN configurations that may have caused the issue.",
            "",
            "Investigation priority for common network scenarios:",
            "• Same VPC, can't connect → SG → NACL → Route Table → Instance state",
            ...
            "• No internet access → Route Table (0.0.0.0/0) → IGW/NAT-GW state → SG → NACL",
            "• VPN connectivity → VPN tunnel status → VGW routes → VPC routes → SG",
            "• Cross-region → TGW peering / VPN / Peering → routes on both sides → SG",
            "",
            "#### Step 3: Exhaustive Investigation Rule",
            "- If one source returns no useful data, try the NEXT related source.",
            "- NEVER conclude '원인을 알 수 없습니다' until you have checked ALL relevant",
            "  log groups, metrics sources, AND resource status for the identified service.",
            "- After exhaustive search with no findings, explicitly list EVERY source you",
            "  checked so the user knows the investigation was thorough.",
            "- If a log group exists but has no recent data, report that fact — it may",
            "  indicate a logging configuration issue itself.",
            "",
            ...
            "This ensures the report renders correctly in both web chat and Slack.",
            "",
            "### Template A: 인프라 현황",
            "```",
            "📊 *인프라 현황 리포트*",
