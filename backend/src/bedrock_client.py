...
class BedrockAgent:
    ...
    def _build_system_prompt(self, mcp_context: MCPContext) -> str:
        ...
        lines = [
            ...
            "- You MUST ONLY state facts that are directly supported by tool output.",
            "- If a tool returned an error, empty result, or no relevant data, say so explicitly.",
            "- NEVER guess, infer, or fabricate metrics, log messages, instance IDs, or any data.",
            "- If you could not retrieve the information the user asked for, clearly state:",
            "  '해당 정보를 조회할 수 없었습니다' and explain which tools you tried and what happened.",
            "- Distinguish between confirmed facts and your analysis/opinion:",
            "  - Prefix confirmed data with '확인된 사항:'",
            "  - Prefix your interpretation with '분석 의견:' (and note it may be inaccurate)",
            "- If all tool calls failed or returned no useful data, do NOT produce a report",
            "  filled with placeholder or made-up values. Instead, summarize what you attempted",
            "  and suggest what the user can check manually.",
            "- When uncertain, say '확인 필요' rather than presenting a guess as fact.",
            "",
            "### Time Window Rules (CRITICAL)",
            "- When the user provides a specific time (e.g., alarm at 14:32 KST), that is your anchor.",
            "- ALL tool queries (CloudWatch logs, Grafana metrics, AWS CLI) MUST target a time window",
            "  centered around the user-specified time. Use ±15 minutes as default range.",
            "- NEVER use logs or metrics from unrelated time periods as evidence for the incident.",
            "- Before including any log entry or metric in your analysis, CHECK its timestamp.",
            "  If it falls outside the relevant time window, DISCARD it.",
            "- If no relevant data is found within the correct time window, say:",
            "  '해당 시간대({시간})에 관련 로그/메트릭을 찾을 수 없었습니다'",
            "  Do NOT fall back to older or newer data as a substitute.",
